...
Нам не важно на каком языке пишется бэк или клиент, генератор поддердивает много популярных языков и библиотек. Есть генераторы для сервера и генераторы для
клиента
...
1. **Параллельная разработка:** API-first подразумевает установку контракта. Создание контракта между службами, который соблюдают команды по всей организации,
   позволяет этим командам работать над несколькими API одновременно. Разработчикам не нужно ждать выпуска обновлений API перед переходом к следующему. Команды
   могут создавать mock API и тестировать зависимости от API на основе установленного определения API (Spring Cloud Contract).
...
OpenAPI (OpenAPI Specification) - это набор правил, описывающих структуру и формат файла, который описывает RESTful API. Файл обычно имеет формат YAML (.yml)
или JSON (.json). Вот основные сущности, которые можно найти в файле OpenAPI в формате YAML:
...
3. mkp-callcenter-api

## Про генерацию open api файла

1. http://localhost:8085/v3/api-docs
2. swagger-cli bundle openapi.json -o merged.json

## **Про что почитать и куда покопать**

1. Пример использования api first в spring
   boot[API First Development with Spring Boot and OpenAPI 3.0 | Baeldung](https://www.baeldung.com/spring-boot-openapi-api-first-development)

2. OpenAPI/Swagger для начинающихhttps://habr.com/p/776538/
...
4. Так же есть возможность применения API First для Асинхронного взаимодействия!https://habr.com/p/768012/Интересно

5. https://www.asyncapi.com/docs/tutorials/getting-started/hello-world

6. [OpenAPI станет проще: готовится версия 4.0 / Хабр](https://habr.com/ru/companies/ruvds/articles/779788/)
